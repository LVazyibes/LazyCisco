...
class SearchDialog(QDialog):
    ...
    def init_ui(self):
        ...
        if self.search_mode == 'include':
            description = "Interface konfigürasyonunda BULUNAN metni arayın:"
            placeholder = "Örnek: 'switchport mode access' veya 'vlan 10' gibi konfigürasyon satırları"
        else:
            description = "Interface konfigürasyonunda BULUNMAYAN metni arayın:"
            placeholder = "Örnek: 'shutdown' veya 'description' gibi konfigürasyon satırları"
...
class TemplateSelectionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(QLabel("Kullanılacak şablonu seçin:"))
        
        # Use QTreeWidget for hierarchical display
        self.tree_widget = QTreeWidget()
        self.tree_widget.setHeaderLabel("Şablonlar")
...
class TemplateManagerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Şablon Yönetimi")
        self.setModal(True)
        self.resize(800, 600)
        ...
        left_layout.addWidget(QLabel("Şablonlar:"))
        
        # Template tree
        self.tree_widget = QTreeWidget()
        self.tree_widget.setHeaderLabel("Şablonlar ve Klasörler")
        ...
        add_button = QPushButton("Yeni Şablon")
        ...
        right_layout.addWidget(QLabel("Şablon İçeriği:"))
        
        self.content_area = QTextEdit()
        self.content_area.setPlaceholderText("Bir şablon seçin veya yeni bir şablon oluşturun...")
    ...
    def add_new_template(self):
        ...
        template_name, ok = QInputDialog.getText(self, "Yeni Şablon", "Şablon adını girin (örn: klasör/isim.txt):")
        if ok and template_name:
            ...
            if self.template_manager.save_template(template_name, ""):
                self.load_templates()
                QMessageBox.information(self, "Başarılı", f"Şablon '{template_name}' oluşturuldu!")
            else:
                QMessageBox.critical(self, "Hata", f"Şablon oluşturulamadı: {template_name}")
    ...
    def display_selected_template_content(self, item, column=0):
        ...
        if item_data and item_data['type'] == 'file':
            ...
            if content is not None:
                ...
                self.content_area.setPlainText("Şablon yüklenemedi.")
                self.current_template = None
        else:
    ...
    def save_edited_template(self):
        ...
        if self.current_template:
            ...
            QMessageBox.warning(self, "Uyarı", "Kaydetmek için bir şablon seçin.")
        ...
        if current_item:
            ...
            if item_data:
                reply = QMessageBox.question(self, "Silme Onayı", 
                                           f"'{item_data['name']}' öğesini silmek istediğinizden emin misiniz?",
                ...
                if reply == QMessageBox.Yes:
                    if self.template_manager.delete_template(item_data['path']):
                        ...
                        self.content_area.setPlainText("")
                        self.current_template = None
                        QMessageBox.information(self, "Başarılı", "Öğe silindi!")
                    else:
                        QMessageBox.critical(self, "Hata", "Öğe silinemedi!")
        else:
            QMessageBox.warning(self, "Uyarı", "Silmek için bir öğe seçin.")
    ...
    def rename_selected_template(self):
        ...
        if current_item:
            ...
            if item_data:
                ...
                new_name, ok = QInputDialog.getText(self, "Yeniden Adlandır", f"'{old_name}' için yeni ad:", QLineEdit.Normal, old_name)
                if ok and new_name and new_name != old_name:
                    ...
                    if self.template_manager.rename_template(old_path, new_path):
                        ...
                        QMessageBox.information(self, "Başarılı", "Öğe yeniden adlandırıldı!")
                    else:
                        QMessageBox.critical(self, "Hata", "Öğe yeniden adlandırılamadı!")
        else:
            QMessageBox.warning(self, "Uyarı", "Yeniden adlandırmak için bir öğe seçin.")
...
class InterfaceConfigDialog(QDialog):
    ...
    def __init__(self, connection, interface, parent=None):
        ...
        self.setWindowTitle(f"{interface} Konfigürasyonu")
        self.setModal(True)
        self.resize(600, 400)
    ...
    def initUI(self):
        ...
        template_button = QPushButton("Şablondan Yükle")
        template_button.clicked.connect(self.load_template_to_config_area)
        layout.addWidget(template_button)

        # Configuration area
        layout.addWidget(QLabel("Konfigürasyon Komutları:"))
        self.config_area = QTextEdit()
        self.config_area.setPlaceholderText("Interface konfigürasyon komutlarını buraya girin...")
        ...
        if template_dialog.exec_() == QDialog.Accepted:
            ...
            if selected_template:
                ...
                if template_content:
                    ...
                    QMessageBox.critical(self, "Hata", "Şablon yüklenemedi!")

    def apply_configuration(self):
        """Applies the entered configuration commands to the interface."""
        config_text = self.config_area.toPlainText().strip()
        if not config_text:
            QMessageBox.warning(self, "Uyarı", "Lütfen konfigürasyon komutlarını girin.")
            return

        try:
            commands = [f"interface {self.interface}"] + config_text.split('\n') + ["exit"]
            output = self.connection.send_config_set(commands)
            QMessageBox.information(self, "Başarılı", f"{self.interface} konfigürasyonu uygulandı!")
            self.accept()
        except Exception as e:
...
class BulkInterfaceConfigDialog(QDialog):
    ...
    def __init__(self, connection, interfaces, parent=None):
        ...
        self.setWindowTitle("Toplu Interface Konfigürasyonu")
        self.setModal(True)
        self.resize(600, 400)
    ...
    def initUI(self):
        ...
        info_label = QLabel(f"Seçili Interface Sayısı: {len(self.interfaces)}")
        info_label.setStyleSheet("font-weight: bold; font-size: 14px; margin-bottom: 10px;")
        layout.addWidget(info_label)

        # Show selected interfaces
        interface_list_label = QLabel("Seçili Interface'ler:")
        ...
        layout.addWidget(QLabel("Konfigürasyon Komutları:"))
        self.config_area = QTextEdit()
        self.config_area.setPlaceholderText("Tüm seçili interface'lere uygulanacak konfigürasyon komutlarını buraya girin...")
        layout.addWidget(self.config_area)

        # Buttons
        button_layout = QHBoxLayout()
        
        apply_button = QPushButton("Tümüne Uygula")
    ...
    def apply_bulk_configuration(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Başarılı", 
                                  f"{len(self.interfaces)} interface'e konfigürasyon başarıyla uygulandı!\n\n"
                                  f"Kullanılan komut: {range_command}")
            self.accept()
            
        except Exception as e:
            print(f"DEBUG: Error in bulk configuration: {str(e)}")
            QMessageBox.critical(self, "Hata", 
                               f"Toplu konfigürasyon uygulanırken hata oluştu: {str(e)}\n\n"
                               f"Denenen komut: {range_command if 'range_command' in locals() else 'Bilinmiyor'}")
...
class GlobalConfigDialog(QDialog):
    ...
    def __init__(self, connection, parent=None):
        ...
        self.setWindowTitle("Global Konfigürasyon")
        self.setModal(True)
        self.resize(600, 400)
    ...
    def initUI(self):
        ...
        # Info
        info_label = QLabel("Global Konfigürasyon Komutları")
        info_label.setStyleSheet("font-weight: bold; font-size: 14px; margin-bottom: 10px;")
        layout.addWidget(info_label)
        ...
        layout.addWidget(QLabel("Konfigürasyon Komutları:"))
        self.config_area = QTextEdit()
        self.config_area.setPlaceholderText("Global konfigürasyon komutlarını buraya girin (configure terminal komutu otomatik eklenir)...")
        layout.addWidget(self.config_area)

        # Console output area
        layout.addWidget(QLabel("Konsol Çıktısı:"))
    ...
    def apply_configuration(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Başarılı", "Global konfigürasyon uygulandı!")
...
class SwitchTabWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.gigabit_checkbox = QCheckBox("Tüm Gigabit Arayüzlerini Seç")
        ...
        interface_v_layout.addWidget(QLabel('Interface Listesi'))
        ...
        vlan_v_layout.addWidget(QLabel('VLAN Listesi'))
        ...
        info_layout.addWidget(QLabel('Seçili Interface/VLAN Bilgisi'))
        ...
        self.bulk_apply_button = QPushButton("Seçilenlere Konfigürasyon Uygula")
        ...
        self.command_input.setPlaceholderText("Tek komut girin ve Enter'a basın (konfigürasyon için 'Global Konfigürasyon Uygula'yı kullanın)...")
        ...
        self.apply_global_config_button = QPushButton('Global Konfigürasyon Uygula')
    ...
    def show_context_menu(self, position):
        ...
        if selected_item:
            ...
            if custom_widget:
                ...
                add_template_action = QAction(f"Şablona Ekle", self)
    ...
    def add_interface_to_template(self, interface_name):
        ...
        if self.connection:
            try:
                ...
                template_name, ok = QInputDialog.getText(self, "Şablona Ekle", 
                                                         f"Şablon için bir isim girin (örn: klasör/isim):", 
                ...
                if ok and template_name:
                    ...
                    if self.template_manager.save_template(template_name, config_output):
                        QMessageBox.information(self, "Başarılı", f"Arayüz konfigürasyonu '{template_name}' şablonu olarak kaydedildi!")
                    else:
                        QMessageBox.critical(self, "Hata", f"Şablon kaydedilirken bir hata oluştu: {template_name}")
                elif not template_name and ok:
                    QMessageBox.warning(self, "Uyarı", "Şablon adı boş bırakılamaz.")

            except Exception as e:
                QMessageBox.critical(self, "Hata", f"Arayüz konfigürasyonu alınırken hata oluştu: {str(e)}")
        else:
            QMessageBox.warning(self, "Uyarı", "Önce switch'e bağlanın!")


    def default_interface(self, interface_name):
        """Resets the selected interface to its default configuration."""
        if self.connection:
            reply = QMessageBox.question(self, 'Interface Default', f'Interface "{interface_name}" ayarlarını sıfırlamak istediğinizden emin misiniz?',
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                try:
                    commands = [
                    ...
                    QMessageBox.information(self, "Başarılı", f"Interface {interface_name} varsayılan ayarlarına döndürüldü.")
                    self.load_interfaces()
                except Exception as e:
                    self.output_area.append(f'Hata: {str(e)}')
                    QMessageBox.critical(self, "Hata", f"Interface varsayılan ayarlarına döndürülürken hata: {str(e)}")
        else:
            QMessageBox.warning(self, "Uyarı", "Önce switch'e bağlanın!")
    ...
    def connect_to_switch(self):
        ...
        if self.connection:
            ...
            self.output_area.append(f'Uyarı: Zaten {self.ip} adresine bağlısınız.')
            return
        ...
        try:
            ...
            error_msg = f"Kimlik doğrulama hatası: {self.ip}"
            print(f"DEBUG: {error_msg}")
            self.output_area.append(error_msg)
        except NetmikoTimeoutException:
            error_msg = f"Bağlantı zaman aşımı: {self.ip}"
            print(f"DEBUG: {error_msg}")
            self.output_area.append(error_msg)
        except Exception as e:
    ...
    def backup_current_config(self):
        ...
        if self.connection:
            ...
            if filename:
                self.output_area.append(f'Yedekleme tamamlandı: {filename}')
                QMessageBox.information(self, "Yedekleme", f"Konfigürasyon '{filename}' dosyasına kaydedildi.")
            else:
                self.output_area.append('Yedekleme hatası!')
                QMessageBox.critical(self, "Hata", "Konfigürasyon yedeklenirken hata oluştu.")
        else:
    ...
    def open_bulk_interface_config(self):
        ...
        if not selected_interfaces:
            QMessageBox.warning(self, "Uyarı", "Lütfen en az bir interface seçin.")
            return
    ...
    def load_interfaces(self):
        ...
        if self.connection:
            try:
                ...
                self.output_area.append(f'Hata interface yüklenirken: {str(e)}')
        else:
    ...
    def load_vlans(self):
        ...
        if self.connection:
            try:
                ...
                self.output_area.append(f'Hata VLAN yüklenirken: {str(e)}')
        else:
            self.vlan_list.clear()
    ...
    def show_interface_details(self, item):
        ...
        if custom_widget and self.connection:
            ...
            try:
                ...
                self.interface_info.setPlainText(f'Hata: {str(e)}')
        ...
        if self.connection:
            ...
            if command.lower().startswith("conf") or \
               ...
               command.lower().startswith("configure terminal"):
                self.output_area.append(f'\n> {command}\nKonfigürasyon komutları için lütfen "Global Konfigürasyon Uygula" butonunu kullanın.')
...
class CiscoSwitchGUI(QWidget):
    ...
    def initUI(self):
        ...
        left_layout.addWidget(QLabel('Kayıtlı Switch\'ler (Sürükle-Bırak ile Sırala):'))
        ...
        add_switch_group.addWidget(QLabel('Yeni Switch Ekle/Güncelle:'))
        
        self.ip_input = QLineEdit()
        self.ip_input.setPlaceholderText("IP Adresi")
        ...
        self.manage_templates_button = QPushButton("Şablonları Düzenle")
    ...
    def edit_selected_switch(self):
        ...
        if current_item:
            ...
            # Create edit dialog
            dialog = QDialog(self)
            dialog.setWindowTitle("Switch Düzenle")
            ...
            layout.addWidget(QLabel("Switch Adı (İsteğe bağlı):"))
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                if new_ip and new_username and new_password:
                    ...
                    if self.config_manager.save_switch_credentials(new_ip, new_username, new_password, new_name):
                        QMessageBox.information(self, "Başarılı", "Switch bilgileri güncellendi!")
                        self.load_saved_switches()
                    else:
                        QMessageBox.critical(self, "Hata", "Switch bilgileri güncellenirken hata oluştu!")
                else:
                    QMessageBox.warning(self, "Uyarı", "IP, kullanıcı adı ve şifre alanları boş bırakılamaz!")
    ...
    def delete_selected_switch(self):
        ...
        if current_item:
            ...
            reply = QMessageBox.question(self, 'Silme Onayı', f'{ip} adresli switch\'i silmek istediğinizden emin misiniz?',
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                if self.config_manager.delete_switch(ip):
                    QMessageBox.information(self, "Başarılı", "Switch silindi!")
                    self.load_saved_switches()
                else:
                    QMessageBox.critical(self, "Hata", "Switch silinirken hata oluştu!")
    ...
    def save_switch(self):
        ...
        if ip and username and password:
            ...
            name, ok = QInputDialog.getText(self, "Switch Adı", "Switch için bir isim girin (isteğe bağlı):")
            if not ok:
                name = ""  # User cancelled, use empty name
            
            if self.config_manager.save_switch_credentials(ip, username, password, name.strip()):
                QMessageBox.information(self, "Başarılı", "Switch kaydedildi!")
                self.load_saved_switches()
                self.ip_input.clear()
                self.user_input.clear()
                self.pass_input.clear()
            else:
                QMessageBox.critical(self, "Hata", "Switch kaydedilemedi!")
        else:
            QMessageBox.warning(self, "Uyarı", "IP, kullanıcı adı ve şifre alanlarını doldurun!")
        ...
        if username and password:
            ...
            for i in range(self.tab_widget.count()):
                ...
                if hasattr(tab_widget, 'ip') and tab_widget.ip == ip:
                    ...
                    QMessageBox.information(self, "Bilgi", f"{ip} için tab zaten açık!")
                    return
            ...
            QMessageBox.critical(self, "Hata", f"{ip} için kimlik bilgileri bulunamadı!")
